...
const ProblemView = () => {
  ...
  <FontAwesomeIcon color='var(--theme-color)' icon={faClock} /> {problemDetails?.date.substring(11, 19)} &nbsp;&nbsp;&nbsp;
            ...
            {problemDetails ? <ul class=" mt-3 mb-3 ul-list-css d-md-flex flex-wrap align-items-center justify-content-center text-capitalize ff-open-sans">
              ...
              <li class="mr-4 problem-list-hover-css">
                {problemDetails?.isSolved ?
                  <div><FontAwesomeIcon color='green' icon={faCheckCircle} /> Published&nbsp;&nbsp;&nbsp;</div> :
                  <div>
                    <FontAwesomeIcon color="#8B8000" icon={faExclamationCircle} /> Not Published&nbsp;&nbsp;&nbsp;
                  </div>
                }
            ...
            {problemDetails ? <>{mark && <Loading />}{!mark && (problemDetails?.isSolved ? <p>Mark this article as <button style={{ color: "grey", fontWeight: "bolder" }} className="remove-default" onClick={() => markProblem()}>Not Published</button></p> : <p>Mark this problem as <button className="remove-default" style={{ color: "green", fontWeight: "bolder" }} onClick={() => markProblem()}>Published</button></p>)}</> : <Skeleton />}

            {problemDetails ? (<><p class="font-weight-normal mt-5 mb-5"><h6 className='problem-list-hover-css'><FontAwesomeIcon color='var(--theme-color)' icon={faInfoCircle} />&nbsp;Article Desciption:</h6>&nbsp;&nbsp;&nbsp;{problemDetails?.description}</p>
              <button className='button-css'><a target="_blank" style={{ textDecoration: "none", color: "white" }} href={problemDetails?.link}>Resources</a></button></>) : <Skeleton />}
          </div>
        </div>
